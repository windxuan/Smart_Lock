...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagDOOR)
    {
        info->event[info->num] = EVENT_DOOR;
        info->num++;
        dataPoints->valueDOOR = gizStandardDecompressionValue(DOOR_BYTEOFFSET,DOOR_BITOFFSET,DOOR_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueDOOR != cur->valueDOOR)
    {
        GIZWITS_LOG("valueDOOR Changed\n");
        ret = 1;
    }

    if(last->valueTEM != cur->valueTEM)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueTEM Changed\n");
            ret = 1;
        }
    }
    if(last->valueWEM != cur->valueWEM)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueWEM Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(DOOR_BYTEOFFSET,DOOR_BITOFFSET,DOOR_LEN,(uint8_t *)devStatusPtr,dataPoints->valueDOOR);
    gizByteOrderExchange((uint8_t *)devStatusPtr->wBitBuf,sizeof(devStatusPtr->wBitBuf));


    devStatusPtr->valueTEM = exchangeBytes(gizY2XFloat(TEM_RATIO,  TEM_ADDITION, dataPoints->valueTEM));     
    devStatusPtr->valueWEM = exchangeBytes(gizY2XFloat(WEM_RATIO,  WEM_ADDITION, dataPoints->valueWEM));     
